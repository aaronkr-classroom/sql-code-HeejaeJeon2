...
2. 데이터웨어하우스, 데이터마트나 데이터마이닝에서는 수집된 데이터를 분석이 용이하도록 통일된 형태로 변환하는 과정을 ( 추출)이라고 한다.
...
SQL 데이터베이스는 관계형 모델을 기반으로 데이터를 표 형식으로 저장하고, 트랜잭션 관리와 ACID 원칙을 지원합니다.
  주로 정형 데이터를 처리합니다. 반면, NoSQL 데이터베이스는 비정형, 반정형 데이터를 저장하고, 확장성이 뛰어나며,
  유연한 스키마를 제공합니다. NoSQL은 주로 대용량의 데이터 처리와 빠른 읽기/쓰기 작업을 요구하는 경우에 사용됩니다.
